...
app.controller("login", ['$scope', '$http', '_EZintersocket', function(_scope){
	_scope.username = "";
	_scope.password = "";
	_scope.submit = function(event){
		event.preventDefault();
		_http.post("login", {'username' : _scope.username.trim(), 'password' : _scope.password}, function(result){
			var response = JSON.parse(result.responseText);
			if(response.status === "ok"){
				document.cookie = "token=" + response.cookie + "; path=.";
				window.location.href = response.referal;
			}
